...
- [ ] **Step 5: No tocar la version del sobre**

`exportFile()` escribe `{app:"aures", v:1, audio:{...}, data:snapshot()}`. La `v` describe la forma del sobre, no la del contenido, y el sobre no cambia. `importFile()` ni siquiera la lee: valida `app`, `data` y la correspondencia con el audio. Un lector antiguo ignora la clave `tempo` que no conoce y uno nuevo la da por vacia cuando falta, asi que en las dos direcciones el archivo sigue cargando. `v` se queda en 1.

- [ ] **Step 6: Comprobar sintaxis y comportamiento**
...
4. Un documento sin la clave: copiar el `.aures.json` exportado, borrarle a mano la linea `"tempo"` del bloque `data`, y arrastrarlo sobre la ventana. Debe cargar sin error y dejar `S.tempo` en `[]`. Es el caso de todo archivo guardado antes de este cambio.
...
`BANDS` no cambia de forma, pero sus alturas son literales escritos contra `RUL`, no contra `LANE`, asi que hay que editarlas: sin esto las dos bandas siguen dibujandose donde estaban y la de tempo se pinta encima de la de secciones. Quedan asi:
...
4. Los marcadores de seccion y acorde que ya hubiera siguen cayendo en su franja y se pueden arrastrar. Si se dibujan desplazados, `BANDS` se quedo sin editar en el paso 1.
5. Encoger la ventana en vertical hasta dejar la onda muy baja: `LANE` mide ahora 14 px mas y la onda tiene menos sitio. Comprobar que no desaparece ni se dibuja invertida antes de que la ventana sea absurdamente pequena.
...
// a series dies of old age, and also when its anchor is gone: an undo, an
// import or another audio replace S.tempo wholesale and leave tapAnchor
// pointing at a detached object that no drawing would ever see again
function seriesAlive(){
  return taps.length > 0 &&
         performance.now() - tapWall <= TAP_GAP &&
         S.tempo.includes(tapAnchor);
}

function tap(){
  const now = audio.currentTime;
  if(!seriesAlive()){ taps = []; tapAnchor = null; }
...
- [ ] **Step 4: Cortar la serie al abrir otro audio**

`seriesAlive()` cubre el caso general, pero `load()` es explicito y barato. En la linea que la tarea 2 dejo asi:

```js
  S.markers = []; S.chords = []; S.tempo = []; S.loop = {on:false, s:null, e:null};
```

anadir a continuacion:
...
7. Empezar una serie, `Ctrl+Z` a mitad, y seguir golpeando dentro de los 3 s: los golpes siguientes abren un anclaje nuevo en vez de escribir en el que el deshacer acaba de tirar. Sin `seriesAlive()` los golpes irian a un objeto que ya no esta en `S.tempo` y no se veria nada.
8. Empezar una serie y, antes de 3 s, arrastrar otro audio sobre la ventana: el primer `T` en la cancion nueva crea un anclaje, no continua la serie anterior.
...
/* six controls in a 250 px panel: the row is tighter than the marker ones */
#tlist .mk{gap:4px;padding:5px 6px}
#tlist .t{min-width:42px;color:#b9c8e0}
#tlist .num{width:44px;font-size:11px;padding:2px 4px}
#tlist .beats{width:26px}
#tlist .x{padding:2px 5px;font-size:11px}
```

Las cuentas, porque el panel no perdona: `aside` mide 250 px fijos y la fila lleva seis controles. 42 de la hora, 44 del BPM, 26 de los pulsos y tres botones de unos 22, mas cinco huecos de 4 y 12 de padding, son 206 px. Con la anchura de `.mk` que usan las otras listas y `.num` a 46 se pasaba de 250 y los botones caian fuera. Si al implementar la suma no cuadra, lo que se estrecha es la hora, no los botones.
...
function renderTempoList(){
  ...
  S.tempo.forEach((a, i) => {
    ...
    beats.className = "num beats";                 // narrower: it holds one digit
    beats.value = a.beats;
...
Tres ediciones, no una. En **What it does**, insertar una entrada nueva despues de la de los marcadores, con la misma forma en negrita que las demas:
...
En esa misma lista, la entrada de deshacer ya no dice la verdad, porque el deshacer cubre ahora tambien los anclajes:

```markdown
- **Undo and redo** for markers and tempo anchors, so a mistaken delete costs one `Ctrl+Z` rather than an evening
```

Y en la tabla de **Keyboard**, una fila detras de la de `C`:
...
Este paso es de mykl y no de un agente: la extension de Chrome no conecta en esta maquina, comprobado el 2026-08-25 y otra vez el 2026-08-30.

Abrir `demo.mp3`, tomar el tempo con `T`, dejar visibles la banda de tempo con su anclaje y la rejilla sobre la onda, y rehacer `screenshot.png` con el mismo encuadre que la actual. Las capturas se hacen siempre contra `demo.mp3`, nunca contra audio real.

El texto alternativo de la imagen en el README nombra las bandas y se queda corto. Pasa a:
...
2. `README.md` renderizado en GitHub: la entrada nueva de **What it does**, la de deshacer corregida, la fila de `T` en la tabla y el texto alternativo de la imagen.
3. La captura muestra la banda de tempo y la rejilla.
4. Buscar `T` en el README y en la modal: la tecla esta documentada en los dos sitios donde alguien la buscaria.
